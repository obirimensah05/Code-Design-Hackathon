...
window.PV_DATA = { TOOLS, FIELDS, LEVELS, GOALS, CREATORS, VIDEOS, makeLeaderboard };
window.PV_DATA.byTool = (id) => VIDEOS.filter(v => v.toolId === id);
window.PV_DATA.byLevel = (toolId, level) => VIDEOS.filter(v => v.toolId === toolId && v.level === level);
window.PV_DATA.creator = (id) => CREATORS.find(c => c.id === id);
window.PV_DATA.tool = (id) => TOOLS.find(t => t.id === id);
window.PV_DATA.video = (id) => VIDEOS.find(v => v.id === id);
window.PV_DATA.fmtViews = (n) => n >= 1000 ? `${(n/1000).toFixed(n>=10000?0:1)}k` : `${n}`;
